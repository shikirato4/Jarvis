...
def test_ui_click_target_uses_visual_grounding(jarvis_app) -> None:
    jarvis_app.runtime_service.switch_mode("operator", reason="ui visual click")
    receipt = jarvis_app.runtime_service.ui_click_target({"label": "Guardar", "kind": "button", "approved": True})
    backend = jarvis_app.ui_automation_service._backend  # noqa: SLF001
    assert receipt.success is True
    assert receipt.data["matched_label"] == "Boton Guardar"
    assert receipt.data["match_source"] in {"element", "text"}
...
def test_ui_write_returns_blocked_receipt_outside_operator_for_untrusted_window(jarvis_app) -> None:
    receipt = jarvis_app.runtime_service.ui_write_text({"text": "Hola Editor", "mode": "copilot"})
    assert receipt.success is False
    assert receipt.status.value == "blocked"
    assert receipt.security_decision == "blocked"
    events = jarvis_app.event_bus.recent_events()
    assert any(event["event_name"] == "ui.blocked" for event in events)


def test_interface_write_text_action_does_not_raise_when_mode_is_invalid(jarvis_app) -> None:
    receipt = jarvis_app.runtime_service.execute_action("interface.write_text", {"text": "Hola Editor", "mode": "copilot"})
    assert receipt.status.value == "failed"
    assert receipt.data["success"] is False
    assert receipt.data["status"] == "blocked"
