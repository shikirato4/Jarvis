...
def test_autonomy_unity_write_requires_mission_control(tmp_path: Path) -> None:
    ...
    app.start()
    try:
        receipt = app.runtime_service.autonomy_start({"goal": "Write a Unity script", "autonomy_level": "supervised_autonomous"})
        mission = app.autonomy_service_runtime._state.get(receipt.mission_id)  # noqa: SLF001
        assert mission is not None
        mission.plan = MissionPlan(
            mission_id=mission.mission_id,
            summary="unity write plan",
            strategy_name="manual-test",
            steps=[
                MissionStep(
                    step_id="unity-1",
                    kind=MissionStepKind.ACTION,
                    title="Write Unity script",
                    description="Write a script inside a Unity project",
                    target="unity.write_script",
                    payload={
                        "project": str(project),
        ...
        assert stepped.state.pending_approval_step_id == "unity-1"
    finally:
        app.stop()
