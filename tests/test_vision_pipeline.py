...
def test_ui_awareness_action_serializes_without_binary_payload(jarvis_app) -> None:
    receipt = jarvis_app.runtime_service.execute_action(
        "vision.ui_awareness",
        {"capture": {"target_type": CaptureTargetType.ACTIVE_WINDOW.value}},
    )
    payload = receipt.model_dump(mode="json")
    encoded = json.dumps(payload)
    assert encoded
    awareness = payload["data"]["awareness_result"]
    assert awareness["text_detected"]
    assert awareness["elements_detected"]
    capture = payload["data"]["capture_result"]
    assert capture["image_bytes"] is None
    assert capture["image_byte_count"] > 0
